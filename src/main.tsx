...
const authConfig: TAuthConfig = {
    ...
    authorizationEndpoint: 'http://localhost:8180/auth/realms/hrm/protocol/openid-connect/auth',
    tokenEndpoint: 'http://localhost:8180/auth/realms/hrm/protocol/openid-connect/token',
    logoutEndpoint: 'http://localhost:8180/auth/realms/hrm/protocol/openid-connect/logout',
...
const UserInfo = (): JSX.Element => {
    ...
    const callPublic = async () => {
        const message = await callEndpoint('http://localhost:8090/public', token);
        setPublicResponse(message);
    }

    const callAdmin = async () => {
        const message = await callEndpoint('http://localhost:8090/employees', token);
